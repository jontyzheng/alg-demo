 ...
 * Leet_209_MinLenPart：
 */
public class Leet_209_MinLenPart {
    public int minSubArrayLen(int target, int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            // 累加 比较大小 如果加到最后一位还是没有的话跳出循环
            // 保留当前子数组（头坐标和长度）有同样满足的从中间选最短的（需要一个 list）
            // new 列表的时候 new 一个 sum 用于累加
            // 如果没有达到要求且后面还有位数的话继续往后加
            int j = i;
            ArrayList<Integer> currentList = new ArrayList<>(); // 新的列表
            int sum = 0;    // 新的累加
            // 模拟没有找到目标值一直往后加的过程
            while (haveNotFound && j < nums.length - 1) {
                sum += nums[j];
                if (sum >= target) {
                    haveNotFound = false;
                    if (currentList.size() >= 1 && currentList.size() < targetList.size()) {
    ...
    static public int minSubArrayLen2(int target, int[] nums) {
        ...
        int leftSum = nums[maxIndex];
        int rightSum = leftSum;
        // 往两头找 从往右找 5 1 8 4 1
        for (int i = maxIndex+1, j = maxIndex-1; i < nums.length | j >= 0; i++, j--) {   // i 递增往后走 j 递减往左走
            if (i <= nums.length-1) {
                leftSum += nums[i];     // leftSum 往左累计
            }
            if (j >= 0) {
                rightSum += nums[j];    // rightSum 往右累计
            }
            cnt++;
    ...
    static public int maxIndex(int[] nums) {
        int maxIndex = 0;
        for (int i = 0; i < nums.length; i++) {
